function [model, succeeded] = try_to_add_interpolation_point_new(model, functions, new_point, new_fvalues, options)
...
new_point_scaled = (new_point - center)/scale_factor_x;
...
succeeded = false;
pols = 1:basis_size;
pols_linear = pols(~is_pivot(1:dimension+1));
if isempty(pols_linear)
    pols_quadratic = pols([false(1, dimension+1), ~is_pivot(dimension+2:end)]);
else
    pols_quadratic = [];
end
max_abs = 0;
pols_linear = pols_linear(end:-1:1);
for n = pols_linear
    value = evaluate_polynomial(pivot_polynomials(n), new_point_scaled);
...
if abs(max_abs) > tol_pivot
    % Try non-pivot points
    pposition = sum(is_pivot(1:max_abs_idx-1)) + 1;
    points_abs = [points_abs(:, 1:pposition-1), new_point, points_abs(:, pposition:end)];
    points_scaled = [points_scaled(:, 1:pposition-1), new_point_scaled, points_scaled(:, pposition:end)];
    fvalues = [fvalues(:, 1:pposition-1), new_fvalues, fvalues(:, pposition:end)];
    % is_pivot(max_abs_idx) = true;
    [points_scaled, indices, smallest_pivot, pivot_polynomials, is_pivot] = ...
                    check_geometry_lu_preserving(points_scaled, basis, tol_pivot);
	% Reordering information AND deleting unused points
    points_scaled = points_scaled(:, indices);
    ...
    % Try last pivot
    n = find(is_pivot, 1, 'last');
    current_value = evaluate_polynomial(pivot_polynomials(n), points_scaled(:, end));
    new_value = evaluate_polynomial(pivot_polynomials(n), new_point_scaled);
    if abs(new_value) > xi_imp*abs(current_value)
        points_abs(:, end) = new_point;
        points_scaled(:, end) = new_point_scaled;
        fvalues(:, end) = new_fvalues;
        is_pivot(n) = true;
        succeeded = true;
    end
end
...
if sum(is_pivot(1:dimension+1)) <= dimension + 1 || sum(is_pivot(dimension+2:end)) < 1
    ...
    % [Q2, R2] = qr(N2');
    % r2_rows = find(sum(abs(R2), 2) > sqrt(eps), 1, 'last');
    % correction = Q2(:, 1:r2_rows)*linsolve(R2(1:r2_rows, :)', -lns(dimension+2:end, :), linsolve_options);

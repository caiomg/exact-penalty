function polynomials = compute_polynomial_models(model)
    ...
    if points_num <= linear_terms || points_num == full_q_terms %|| strcmp(wid, 'cmg:badly_conditioned_system')
        ...
        for k = functions_num:-1:1
            polynomials{k}  = combine_polynomials(model.pivot_polynomials(1:points_num), l_alpha(k, :));
            polynomials{k} = shift_polynomial(polynomials{k}, model.points_shifted(:, model.tr_center)); 
        end
    end
end
